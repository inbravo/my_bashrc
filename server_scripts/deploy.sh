...
echo -e "${BOLD}${CYAN_F}Starting backup procedure for 'LI' database > '/data/zahmed/backup/li_$CURRENTTIME.sql'${NORM}"
...
echo -e "${BOLD}${CYAN_F}Starting backup procedure li jar files > '/data/zahmed/backup/li_jar_$CURRENTTIME.tgz'${NORM}"
tar czf /data/zahmed/backup/li_jar_$CURRENTTIME.tgz -P /var/local/libjars/li*
sleep 1

echo -e "${BOLD}${CYAN_F}Starting backup procedure li sample_date files > '/data/zahmed/backup/li_sample_data_$CURRENTTIME.tgz'${NORM}"
...
echo -e "${BOLD}${CYAN_F}Backing-up li.war to '/data/zahmed/backup/li_$CURRENTTIME.war'${NORM}"
